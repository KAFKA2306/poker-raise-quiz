...
const fetchWithRetry = async (url, responseType = "text") => {
  let lastError;
...
const parseChoicesFromMarkers = (questionText) => {
  ...
  for (let i = 0; i <= found.length - 4; i += 1) {
    ...
    const choices = candidate.map((item, index) => ({
      value: item.label,
      text: cleanOption(text.slice(item.contentStart, candidate[index + 1]?.markerStart ?? text.length)),
    }));
...
const parseChoicesFromTable = (questionText) => {
  const rows = questionText.split("\n").map((line) => line.trim());
  const choices = [];
  let firstRow = -1;
  for (let index = 0; index < rows.length; index += 1) {
    const match = rows[index].match(/^\|\s*([アイウエ])\s*\|\s*(.*?)\s*\|?$/);
    if (!match) continue;
  ...
  return { stem: rows.slice(0, firstRow).join("\n").trim(), choices };
};

const parseQuestion = (questionText) => {
  const parsed = parseChoicesFromMarkers(questionText) || parseChoicesFromTable(questionText);
  if (!parsed) throw new Error("四つの選択肢を抽出できません");
  return parsed;
};

const realImageReferences = (imageSection) => {
...
const rewriteStemWithImages = async ({ stem, images, session, sourceBaseUrl, sessionDir }) => {
  if (images.length === 0) return stem;
  const assetDir = path.join(sessionDir, "assets");
  await mkdir(assetDir, { recursive: true });
  const additions = [];

  for (const image of images) {
    if (/^https?:\/\//.test(image.sourcePath)) {
      additions.push(`![${image.alt}](${image.sourcePath})`);
      continue;
    }
    const fileName = path.basename(image.sourcePath);
    const sourceUrl = `${sourceBaseUrl}/${session.externalSlug}/${image.sourcePath}`;
    const bytes = Buffer.from(await fetchWithRetry(sourceUrl, "arrayBuffer"));
    await writeFile(path.join(assetDir, fileName), bytes);
    additions.push(`![${image.alt}](./data/exams/applied-information/sessions/${session.id}/assets/${fileName})`);
  }
...
for (const session of sourceConfig.sessions) {
  ...
  for (let start = 1; start <= 80; start += 20) {
    const end = start + 19;
    const fileName = `q${pad(start)}-q${pad(end)}.json`;
    const relative = `modules/${fileName}`;
  ...
  await writeJson(path.join(sessionDir, "manifest.json"), {
    ...
    source: {
      ...
      note: "問題文と選択肢は公開転記を入力補助として正規化し、正答はIPA公式解答キーと全問照合した。第三者の解説文は収録していない。",
    },
    transcriptionInput: {
...
console.log(`合計 ${totalQuestions} 問の本番データ生成に成功しました`);
